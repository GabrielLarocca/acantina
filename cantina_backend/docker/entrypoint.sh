...
if [[ ! -z "${PHP_MEMORY_LIMIT}" ]]; then
  sed -i "s,memory_limit = 1024M,memory_limit = ${PHP_MEMORY_LIMIT},g" /etc/php/${PHP_VERSION}/fpm/php.ini
  sed -i "s,opcache.memory_consumption = 1024M,opcache.memory_consumption = ${PHP_MEMORY_LIMIT},g" /etc/php/${PHP_VERSION}/fpm/php.ini
fi
...
if [[ ! -z "${PHP_MAX_REQUESTS}" ]]; then
  sed -i "s,pm.max_requests = 1500,pm.max_requests = ${PHP_MAX_REQUESTS},g" /etc/php/${PHP_VERSION}/fpm/pool.d/laravel.conf
fi
